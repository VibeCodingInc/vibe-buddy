...
if (import.meta.env.DEV && new URLSearchParams(window.location.search).has('meld')) {
  // THE MELD — temporary-shared-cortex experience prototype (design branch only).
  void import('./dev/MeldPrototype').then(({ default: MeldPrototype }) => {
    root.render(
      <React.StrictMode>
        <ErrorBoundary>
          <MeldPrototype />
...
} else if (import.meta.env.DEV && new URLSearchParams(window.location.search).has('mind-proto')) {
